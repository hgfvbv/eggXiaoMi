module.exports = (options, app) => {
    return async (ctx, next) => {
        ...
        const setting = (await ctx.model.Setting.find())[0];

        if (setting.site_status === 0) {  //未关闭网站
            const { service } = ctx;
                ...
                for (let i = 0; i < navMiddle.length; i++) {
                    if (navMiddle[i].relation) {
                        try {
                            ...
                            let relationGoods = await ctx.model.Goods.find({ status: 1, $or: goodsIds }, { title: 1, goods_img: 1, shop_price: 1 }).sort({ sort: 1 });
